...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `turso` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `bchilcott` with the `ORG` of your provider.
   - replace all instances of the `github.com/bchilcott/pulumi-turso` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-turso
  ...
  ignore: []
  ldflags:
  - -X github.com/bchilcott/pulumi-turso/provider/pkg/version.Version={{.Tag}}
...
├── config.yaml
├── metadata.json
├── pulumi-turso_darwin_amd64_v1
│   └── pulumi-resource-turso
├── pulumi-turso_darwin_arm64
│   └── pulumi-resource-turso
├── pulumi-turso_linux_amd64_v1
│   └── pulumi-resource-turso
├── pulumi-turso_linux_arm64
│   └── pulumi-resource-turso
├── pulumi-turso_windows_amd64_v1
│   └── pulumi-resource-turso.exe
└── pulumi-turso_windows_arm64
    └── pulumi-resource-turso.exe
```
